...
import { changeBookingStatusService, getOwnerBookings } from '../../services/bookings'
...
const ManageBookings: React.FC = () => {
  ...
  const changeBookingStatus = async (bookingId: string, status: BookingStatus) => {
    try {
      ...
      const data = await changeBookingStatusService(bookingId, status)
              ...
              <tr key={booking._id} className='border-t border-borderColor text-gray-500'>
                ...
                <td className='p-3'>
                  {booking.status?.[0] === 'PENDING' ? (
                    <select
                      onChange={(e) => changeBookingStatus(booking._id, e.target.value as BookingStatus)}
